<template>
  <AdminLayout>
    <div class="min-h-screen bg-animated-gradient p-6">
      <div class="mb-6">
        <h1 class="text-4xl font-bold text-white mb-2">Edit Permission</h1>
        <p class="text-white/80">Update permission details</p>
      </div>
      ...
      <GlassCard class="text-white">
        <form @submit.prevent="submit">
          <div class="space-y-6">
            ...
            <!-- Current Roles Using This Permission -->
            <div v-if="permission.roles && permission.roles.length > 0" class="p-4 bg-yellow-500/20 rounded-lg">
              <p class="text-sm font-medium mb-2 text-yellow-300">
                ⚠️ This permission is assigned to {{ permission.roles.length }} role(s):
              </p>
              <div class="flex flex-wrap gap-2">
                <span
                  v-for="role in permission.roles"
                  :key="role.id"
                  class="px-3 py-1 glass rounded-full text-sm"
                >
                  {{ role.name }}
                </span>
              </div>
